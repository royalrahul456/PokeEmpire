...
async def cmd_gc_settings(message: Message, db: AsyncSession):
    ...
    nickname = message.from_user.first_name or message.from_user.username or "Admin"
    ...
    text = (
        ...
        f"◈ ────────────────── ◈\n"
        f"Welcome, Administrator <b>{html.escape(nickname)}</b>!\n\n"
...
async def cmd_set_cover(message: Message):
    ...
    if media_msg:
        ...
        try:
            ...
            print(f"Error downloading cover media in cmd_set_cover: {e}")
...
@router.message(F.chat.type == "private", lambda msg: (msg.from_user.id in config.ADMIN_IDS or msg.from_user.id in getattr(config, "CO_OWNER_IDS", []) or msg.from_user.id in getattr(config, "OWNER_IDS", [])) and msg.from_user.id in active_cover_updates)
