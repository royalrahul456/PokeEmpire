...
async def cmd_grps(message: Message, db: AsyncSession):
    ...
    if not chat_ids:
        await message.answer("📭 The bot is not currently in any groups.")
        return

    progress_msg = await message.answer(
        f"⏳ Fetching info for <b>{len(chat_ids)}</b> groups...", parse_mode="HTML"
    )
    ...
    for chat_id in chat_ids:
        try:
            ...
            name = html.escape(chat.title or "Unnamed Group")
            link = None
            if chat.username:
                ...
                try:
                    link_obj = await message.bot.create_chat_invite_link(chat_id)
            ...
            lines.append(f"• ❌ <code>{chat_id}</code> (inaccessible)")

    text = (
        f"🌐 <b>Bot Groups List</b>\n"
        f"───────────────\n"
        f"<blockquote>👥 Total Groups: <b>{active}/{len(chat_ids)}</b></blockquote>\n\n"
        + "\n".join(lines)
    )

    if len(text) > 4000:
        await progress_msg.delete()
        chunk = f"🌐 <b>Bot Groups ({active} groups)</b>\n───────────────\n"
        for line in lines:
...
async def cmd_spam_start(message: Message, db: AsyncSession):
    ...
    for chat_id, name in fetched:
        builder.button(text=f"📣 {name}", callback_data=f"spam_grp_{chat_id}")
    builder.adjust(1)
    ...
    await message.answer(
        "📢 <b>Spam Wizard</b>\n"
        "───────────────\n"
        "Step 1️⃣: Choose the group to spam:",
...
async def cb_spam_group_chosen(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        f"Step 2️⃣: <b>Send me the message</b> to spam in that group:",
        parse_mode="HTML"
    )
...
async def handle_spam_wizard_text(message: Message):
    ...
    if step == "enter_msg":
        ...
        await message.answer(
            ...
            "───────────────\n"
            "✅ Message saved!\n\n"
            "Step 3️⃣: <b>How many times</b> to send it? (1 – 50):",
            parse_mode="HTML"
        )
        ...
        for i in range(count):
            try:
                ...
                failed += 1
                print(f"⚠️ Spam send failed [{i+1}/{count}]: {e}")
...
total = len(open("handlers/admin.py", encoding="utf-8").readlines())
