...
@router.message(Command("slot", "slots", "casino"))
async def cmd_slot(message: Message, db: AsyncSession):
    user_id = message.from_user.id
    parts = message.text.split()
    
    if len(parts) < 2 or not parts[1].isdigit():
        await message.answer("⚠️ Format: <code>/slot &lt;bet_amount&gt;</code>\n<i>(e.g., <code>/slot 500</code>)</i>", parse_mode="HTML")
        return
    ...
    if win_amount > 0:
        ...
        try:
            ...
            await log_transaction(user_id, win_amount, "SLOT_WIN", f"Won Slot Machine ({multiplier}x)", db)
        except Exception:
            pass
    else:
        try:
            from utils.trainer_level import log_transaction
            await log_transaction(user_id, -bet, "SLOT_LOSS", "Lost Slot Machine", db)
        except Exception:
            pass
    ...
    msg = await message.answer("🎰 <b>SLOT MACHINE</b> 🎰\n───────────────\n🔄 [ 🎰 | 🎰 | 🎰 ]\n<i>Spinning the reels...</i>", parse_mode="HTML")
    await asyncio.sleep(0.5)
    await msg.edit_text(f"🎰 <b>SLOT MACHINE</b> 🎰\n───────────────\n🔄 [ {reel1} | 🎰 | 🎰 ]\n<i>Spinning...</i>", parse_mode="HTML")
    await asyncio.sleep(0.4)
    await msg.edit_text(f"🎰 <b>SLOT MACHINE</b> 🎰\n───────────────\n🔄 [ {reel1} | {reel2} | 🎰 ]\n<i>Stopping...</i>", parse_mode="HTML")
    await asyncio.sleep(0.4)

    if win_amount > 0:
        result_text = (
            f"🎰 <b>SLOT MACHINE RESULT</b> 🎰\n"
            f"───────────────\n"
            f"🎯 <b>[ {reel1} | {reel2} | {reel3} ]</b>\n"
            f"🎉 <b>{title}</b>\n\n"
            f"<blockquote>👤 Trainer: <b>{sender_name}</b>\n"
            f"💰 Multiplier: <b>{multiplier}x</b>\n"
            f"💵 Payout: <b>+{win_amount:,} coins</b> (Net: <code>{'+' if net_profit >= 0 else ''}{net_profit:,}</code>)\n"
            f"💳 New Balance: <code>💰 {user.coins:,} coins</code></blockquote>"
        )
    else:
        result_text = (
            f"🎰 <b>SLOT MACHINE RESULT</b> 🎰\n"
    ...
    builder.row(create_styled_button(text="🎰 Spin Again", key="games", callback_data="btn_launch_slots", style="primary"))
